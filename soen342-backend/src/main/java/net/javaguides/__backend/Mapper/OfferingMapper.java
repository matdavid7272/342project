        ...
        public Offering mapToOffering(OfferingDto offeringDto) {
                ...
                Lesson lesson = lessonRepository.findById(offeringDto.getLessonId()).orElseThrow(
                                () -> new ResourceNotFoundException(
                                                "Lesson with ID " + offeringDto.getLessonId() + " not found"));

                // Fetch Instructor entity if instructorId is not null
                Instructor instructor = null;
                if (offeringDto.getInstructorId() != null) {
                        instructor = instructorRepository.findById(offeringDto.getInstructorId()).orElseThrow(
                                        () -> new ResourceNotFoundException("Instructor with ID "
                                                        + offeringDto.getInstructorId() + " not found"));
                }
                ...
                TimeSlot timeSlot = timeSlotRepository.findById(offeringDto.getTimeSlotId()).orElseThrow(
                                () -> new ResourceNotFoundException(
                                                "TimeSlot with ID " + offeringDto.getTimeSlotId() + " not found"));
                ...
                Location location = locationRepository.findById(offeringDto.getLocationId()).orElseThrow(
                                () -> new ResourceNotFoundException(
                                                "Location with ID " + offeringDto.getLocationId() + " not found"));

                // Return new Offering with loaded Lesson, Instructor (which could be null),
